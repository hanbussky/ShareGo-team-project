...
<body>
	...
	<main>
		...
		<div class="container">
			...
			<div>
				<form action="${pageContext.request.contextPath}/board/together/updateArticle" method="post" onsubmit="return updateAction();">
				...
				<!-- 임시 기본값 저장 -->
					...
					<div class="display-flex justify-content-space-between align-items-center">
						<div class="form-group display-flex justify-content-flex-start align-items-center">
							...
							<select name="brd_id2" id="brd_id2" disabled="disabled">
								...
								<option value="1040" ${article.brd_id == 1040 ? 'selected' : ''}>문화생활</option>
								<option value="1050" ${article.brd_id == 1050 ? 'selected' : ''}>취미생활</option>
								<option value="1060" ${article.brd_id == 1060 ? 'selected' : ''}>기타</option>
							</select>
						</div>
					...
					<div class="trade-info-box padding-10px display-flex flex-direction-column justify-content-flex-start align-items-stretch" style="border: 2px solid var(--subtheme); border-radius: 5px;${isAnyoneJoined ? ' opacity: 0.5; pointer-events: none;' : ''}">
						...
						<div class="display-flex justify-content-space-between align-items-center padding-10px">
							<div class="form-group" style="display: flex;">
								<div class="popup-group">
									<input type="hidden" id="reg_id" name="reg_id" value="${article.reg_id }">
							...
							<div class="form-group flex-grow-1 margin-left-10px display-flex justify-content-flex-end align-items-center">
								...
								<input type="text" class="flex-grow-1" name="trd_loc" placeholder="상세한 지역을 기입해주세요" value="${article.trd_loc }" ${isAnyoneJoined ? 'readonly' : '' }>
							</div>
						</div>
						...
						<div class="form-group display-flex justify-content-space-between align-items-center padding-10px">
							<div class="form-group display-flex justify-content-flex-start align-items-center">
								...
								<fmt:formatDate var="dateValue" value="${article.trd_enddate }" pattern="yyyy-MM-dd hh:mm:ss"/>
							...
							<div class="form-group display-flex justify-content-space-between align-items-center padding-10px">
								...
								<%-- <input type="number" class="font-size-18px font-weight-bolder" name="trd_cost" value="${article.trd_cost == null ? 0 : article.trd_cost }" min="0" readonly="readonly" ${isAnyoneJoined ? 'readonly' : '' }> --%>
								<input type="text" class="font-size-18px font-weight-bolder" name="trd_cost" value="가격을 설정할 수 없습니다" readonly="readonly">
							</div>
						</div>
						...
						<div class="form-group display-flex justify-content-space-between align-items-center padding-10px">
							<div class="form-group display-flex justify-content-flex-end align-items-center">
								...
								<input type="number" class="width-50px" name="trd_max" min="2" value="${article.trd_max }" required="required" ${isAnyoneJoined ? 'readonly' : '' }>
							</div>
							...
							<div class="form-gender display-flex justify-content-flex-start align-items-center">
								...
								<select name="trd_gender" ${isAnyoneJoined ? 'disabled' : '' }>
									<option value="" ${article.trd_gender == null ? 'selected' : '' }>제한 없음</option>
									<option value="201" ${article.trd_gender == 201 ? 'selected' : '' }>남자</option>
									<option value="202" ${article.trd_gender == 202 ? 'selected' : '' }>여자</option>
								</select>
							</div>
							...
							<div class="form-age display-flex justify-content-flex-end align-items-center">
								...
								<input type="number" class="width-50px" name="trd_minage" min="1" max="100" value="${article.trd_minage }" ${isAnyoneJoined ? 'readonly' : '' }>
								<span class="margin-hor-5px font-weight-bolder">~</span>
								<input type="number" class="width-50px" name="trd_maxage" min="1" max="100" value="${article.trd_maxage }" ${isAnyoneJoined ? 'readonly' : '' }>
							</div>
						</div>
					</div>
					...
					<div class="button-group">
						...
						<button type="button" class="btns-cancel" onclick="location.href='${pageContext.request.contextPath}/board/together/${article.art_id}?brd_id=${article.brd_id}&category=${category}';">취소</button>
					</div>
				</form>
			</div>
		</div>
